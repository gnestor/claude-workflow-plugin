...
deno run --allow-net --allow-env --allow-read \
  ...
  page-report <property-id> /pages/about 90daysAgo today
```

### From Google Search Console
Get search queries for specific URL:
```bash
deno run --allow-net --allow-env --allow-read \
  .claude/skills/google/search-console/scripts/search-console-client.ts \
  url-queries sc-domain:example.com https://example.com/pages/about 90
```
