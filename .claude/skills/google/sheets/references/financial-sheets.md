...
**Example workflow:**
```bash
# Get full P&L sheet data
sheets-client.ts get 1o_jWQs1KjRN1aEqFDH0B1hIsq5Lye_Nu030kuvizVOs "'P&L Statement'!A1:Z100"

# Process the data to:
# 1. Identify column headers (dates/periods)
# 2. Identify profit/loss row
# 3. Extract values for comparison periods
...
For calculations, use `--unformatted` to get numeric values directly.
...
These are read-only scopes to ensure data safety.
